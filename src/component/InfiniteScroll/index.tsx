...
const InfiniteScroll = React.forwardRef<HTMLDivElement, InfiniteScrollInfo>(
  (
    {
      children,
      getProducts,
    ...
    const setVisibleProductListByStartRowIndex = useCallback(() => {
      ...
      const currentLastIndexPage = Math.floor(
        newLastIndex / visibleListCapacity
      );
    ...
    const handleScrollFetch = async (e: CustomScrollEvent) => {
      ...
      if (!pageMap[currentFirstIndexPage] && pageMap[currentLastIndexPage]) {
        ...
        if (!requestedPageMap[currentLastIndexPage]) {
          ...
          pageMap[currentLastIndexPage] = await getProducts(
            currentLastIndexPage
          );
        } else {
          return;
        }
    ...
    return (
      <div style={{ height: productHeight * maximumRowLength }} ref={ref}>
        <div
          style={{
            display: "flex",
            flexDirection,
